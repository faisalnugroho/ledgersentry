...
def normalize(raw, documents, manifest, requirement_count):
...
class LedgerSentry(gl.Contract):
    ...
    def open_dispute(self, audit_id: str) -> None:
        ...
        # Node-assigned, non-manipulable clock; deadline = now + window.
        record["dispute_deadline"] = parse_iso_epoch(gl.message_raw["datetime"]) + record["window_seconds"]
        self._save(record)
    ...
    def resolve(self, audit_id: str) -> None:
        ...
        # The dispute response window is a guard BEFORE resolution: a DISPUTED
        # audit can never be resolved before its deadline (node time).
        if record["status"] == STATUS_DISPUTED:
            require(parse_iso_epoch(gl.message_raw["datetime"]) >= record["dispute_deadline"],
        ...
        def leader():
            ...
            prompt = (
                ...
                "reason (10-800 chars), citations [{source: int, quote: verbatim 20-400 "
                "chars}] with quotes that are exact substrings of fetched sources. "
            ...
            try:
                return normalize(gl.nondet.exec_prompt(prompt, response_format="json"),
                                 documents, manifest, len(requirements))
            except Exception:
